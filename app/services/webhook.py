...
class WebhookHandler:
    ...
    def verify_signature(self, signature: str, payload: bytes) -> bool:
        ...
        if not self.webhook_secret:
            ...
            return True  # Skip verification if no secret is set (useful for local testing)
        ...
        try:
            ...
            logger.exception(f"Exception occurred during webhook signature verification: {str(e)}")
            return False
    ...
    async def handle_pr_event(payload: dict):
        ...
        try:
            ...
            logger.info(f"Received PR event: action={pr_action}, repo={repo_full_name}, PR=#{pr_number}")
            ...
            logger.info(f"Processing PR event for repository '{repo_full_name}' PR #{pr_number}")
            ...
            update_response = await github_client.update_pr_description(repo_full_name, pr_number, summary)
            ...
            comment_response = await github_client.add_pr_comment(repo_full_name, pr_number, review_comment)
